...
public class MedianOfTwoSortedArrays {
    ...
    class Solution {
        ...
        private int getNum(int[] nums1, int start1, int end1, int[] nums2, int start2, int end2, int k) {
            ...
            // Math.min(k / 2 - 1,length1)防止数组长度小于k/2
            int i = start1 + Math.min(k / 2, length1) - 1;
            int j = start2 + Math.min(k / 2, length2) - 1;
            if (nums1[i] < nums2[j]) {
                return getNum(nums1, i + 1, end1, nums2, start2, end2, k - (i - start1 + 1));
            } else {
                return getNum(nums1, start1, end1, nums2, j + 1, end2, k - (j - start2 + 1));
            }
        }
    }
